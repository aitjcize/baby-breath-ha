...
ADDON_NAME="baby_respiration_dev"
...
LOCAL_DIR="${REPO_DIR}/baby_respiration"
...
# Dev-ify config.yaml (slug/name/version/panel title) and write a dev build.yaml.
# The add-on is ingress-only, so no port remapping is needed for coexistence
# with the production add-on.
ssh "${HA_HOST}" "cd ${REMOTE_DIR} && \
  sed -i 's/^slug:.*/slug: \"${ADDON_NAME}\"/' config.yaml && \
  sed -i 's/^name:.*/name: \"Baby Respiration Monitor (Dev)\"/' config.yaml && \
  sed -i 's/^version:.*/version: \"dev\"/' config.yaml && \
  sed -i 's/^panel_title:.*/panel_title: Baby Respiration Dev/' config.yaml"
...
docker buildx build "${LOCAL_DIR}" \
  ...
  --label io.hass.name="Baby Respiration Monitor (Dev)" \
  --label io.hass.description="Experimental camera breathing-motion monitor (dev build)" \
  --label io.hass.url="https://github.com/aitjcize/baby-breath-ha" \
  --load
